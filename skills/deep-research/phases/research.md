...
**Don't poll `notes/` — it is empty by construction until an agent finishes**, because each writes one file at the end. You are notified on completion; that is the signal. *Observed:* ~15 background waits, several of 600s, against agents that finished in 4.9–7.2 minutes.
